...
public class MbtiAnotherMethod{
	...
	int [] question = new int [serveQuestion.length];
	for(int index = 0 ; index < question.length ; index++){
	if(question[index] == "A" || question[index] == "a"){
               int[] storeA = question[index] ;
               if(question[index] == "B" || question[index] == "b"){
                   int storeB = question[index] ;
                           else System.out.print("Enter a valid input: ");
               }
	}
